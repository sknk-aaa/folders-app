...
export const useBackupStore = create<BackupStore>((set, get) => ({
  ...
  backup: async () => {
    if (get().state !== 'idle') return { ok: false, error: tr({ en: 'Processing in progress', ja: '処理中です' }) }
    set({ state: 'backing-up' })
    try {
      const manifest = await runBackup(getBackupStorage())
      return { ok: true, manifest }
    } catch (e) {
      return { ok: false, error: e instanceof Error ? e.message : tr({ en: 'Backup failed', ja: 'バックアップに失敗しました' }) }
    } finally {
      set({ state: 'idle' })
    }
  },
  ...
  restore: async () => {
    ...
    try {
      ...
      return { ok: false, error: e instanceof Error ? e.message : tr({ en: 'Restore failed', ja: '復元に失敗しました' }) }
    } finally {
      set({ state: 'idle' })
    }
  },
